...
for distance in [100, 200, 500, 1000]:
    for i in range(25):
        logging.info(f"{distance} m - run {i}")
        ...
        logging.info(
            "Identifying co-locations within (%s km, %s)", X_TOLERANCE, T_TOLERANCE
        )
        ...
        if SAVE_COLOCS:
            ...
            all_observation_combinations.to_csv(
                f"data/repeats/epr_coloc_{distance}_{i}.csv"
            )
